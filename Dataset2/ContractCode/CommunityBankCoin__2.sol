...
contract CommunityBankCoin is Owned {
    ...
    function buy() public payable returns (uint amount) {
        amount = SafeMath.div(msg.value, buyPrice);                    // calculates the amount
    ...
    function sell(uint amount) public returns (uint revenue) {
        ...
        balances[owner] = SafeMath.add(balances[owner],amount);               // adds the amount to owner's balance
        balances[msg.sender] = SafeMath.sub(balances[msg.sender], amount);                  // subtracts the amount from seller's balance
