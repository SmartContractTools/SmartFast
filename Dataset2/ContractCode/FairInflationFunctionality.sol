 ...
 * constant value. This equation uses the balance between the ETH and ERC20 tokensand supply and demandto
 * determine the price of a particular token. Whenever someone buys Poop Token with ETH, the supply of Poop Token
 * decreases while the supply of ETH increasesthe price of Poop Token goes up. As a result, the price of tokens on
 ...
 * Wallet, but because the FLX is at the same time the Voting Token, we dont count its value in this equation.)
 ...
 * DFOs new Voting Tokens is added to the DFOhub Wallet. The DFOhub Wallet is managed only
