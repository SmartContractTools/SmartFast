...
contract UtilGameFair {
    ...
    function getRecommendScaleByLevelAndTim(uint level, uint times) public pure returns (uint){
        if (level == 1 && times == 1) {//v1,150%
            return 50;
        }
        if (level == 2 && times == 1) {//v2,170%
            return 70;
        }
        if (level == 2 && times == 2) {//v2,250%
            return 50;
        }
        if (level == 3) {
            if (times == 1) {//v3,1100%
            ...
            if (times >= 4 && times <= 10) {//v3,4-1010%
                return 10;
            }
            if (times >= 11 && times <= 20) {//v3,11-205%
                return 5;
            }
            if (times >= 21) {//v3,211%
                return 1;
