...
contract TLDR is ScribeRole, ERC20 { // TLDR: Curated covenant & escrow scriptbase with incentivized arbitration 
    ...
    // lexDAO reference for lexScribe reputation governance fees ()
    ...
    ***************/   
    ...
    // lexScribes can submit ether () value for TLDR reputation and special TLDR function access (TLDR write privileges, rdr dispute resolution role) 
    function submitETHreputation() payable public onlyScribe icedown {
        require(msg.value == 0.1 ether); // tenth of ether () fee for refreshing reputation in lexDAO
        
	    reputation[msg.sender] = 3; // sets / refreshes lexScribe reputation to '3' max value, 'three strikes, you're out' buffer
        
	    address(lexDAO).transfer(msg.value); // transfers ether value () to designated lexDAO (0x) address
    }
