...
class ConstantFunctionsState(AbstractDetector):
    ...
    ARGUMENT = 'constant-function-state'  # run the detector with smartfast.py --ARGUMENT
    HELP = 'Constant functions changing the state'  # help information
    IMPACT = DetectorClassification.MEDIUM
    CONFIDENCE = DetectorClassification.PROBABLY

    WIKI = 'https://github.com/SmartContractTools/SmartFast/wiki/Detector-Documentation#constant-functions-changing-the-state'
            ...
            for f in c.functions:
                ...
                if f.view or f.pure:
                    ...
                    if variables_written:
                        attr = 'pure' if f.pure else 'view/constant'
