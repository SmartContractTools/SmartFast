...
class UnimplementedFunctionDetection(AbstractDetector):
    ...
    WIKI = "https://github.com/SmartContractTools/SmartFast/wiki/Detector-Documentation#unimplemented-functions"
...
All unimplemented functions must be implemented on a contract that is meant to be used."""
    ...
    def _match_state_variable(contract, f):
        # print([s.full_name for s in contract.state_variables])
        # print([s.signature_str_name for s in contract.state_variables])
        # print(f.full_name_all )
        # print(f.full_name)
        # return any(s.signature_str_name == f.full_name_all for s in contract.state_variables)
    ...
    def _detect_unimplemented_function(self, contract):
        ...
        for f in contract.all_functions_called:
            if (
                ...
                if self.smartfast.solc_version not in older_solc_versions:
                    # Since 0.5.1, Solidity allows creating state variable matching a function signature
