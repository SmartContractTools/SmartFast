...
class ReentrancyNoGas(Reentrancy):
    ...
    ARGUMENT = 'reentrancy-limited-gas'
    HELP = 'Reentrancy vulnerabilities through send and transfer'
    IMPACT = DetectorClassification.INFORMATIONAL
    CONFIDENCE = DetectorClassification.PROBABLY

    WIKI = 'https://github.com/SmartContractTools/SmartFast/wiki/Detector-Documentation#reentrancy-vulnerabilities-limitedgas'

    WIKI_TITLE = 'Reentrancy vulnerabilities'
    WIKI_DESCRIPTION = '''
Detection of the [reentrancy bug](https://github.com/trailofbits/not-so-smart-contracts/tree/master/reentrancy).
Only report valuable reentrancy that is based on `transfer` or `send`.'''
    ...
    def _detect(self):
        ...
        for (func, calls, send_eth), varsWritten in result_sorted:
            ...
            varsWritten = [FindingValue(v, node, nodes) for (v, node, nodes)
                           in varsWritten if isinstance(v, Variable)]
