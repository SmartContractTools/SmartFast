import re
import logging
from smartfast.smartir.operations import (
    Send,
    Transfer,
    OperationWithLValue,
    HighLevelCall,
...
from smartfast.core.solidity_types import UserDefinedType, MappingType
from smartfast.core.declarations import Enum, Contract, Structure, Function
from smartfast.core.solidity_types.elementary_type import ElementaryTypeName
from smartfast.core.variables.local_variable import LocalVariable
from smartfast.formatters.exceptions import FormatError, FormatImpossible
from smartfast.formatters.utils.patches import create_patch

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("Smartfast.Format")
...
def custom_format(smartfast, result):
    ...
    for element in elements:
        ...
        _patch(smartfast, result, element, target)
...
def _name_already_use(smartfast, name):
    # Do not convert to a name used somewhere else
    if not KEY in smartfast.context:
        all_names = set()
        ...
        smartfast.context[KEY] = all_names
    return name in smartfast.context[KEY]


def _convert_CapWords(original_name, smartfast):
    ...
    if _name_already_use(smartfast, name):
...
def _convert_mixedCase(original_name, smartfast):
    name = original_name
...
def _convert_UPPER_CASE_WITH_UNDERSCORES(name, smartfast):
    if _name_already_use(smartfast, name.upper()):
...
def _get_from_contract(smartfast, element, name, getter):
    contract_name = element["type_specific_fields"]["parent"]["name"]
    contract = smartfast.get_contract_from_name(contract_name)
...
def _patch(smartfast, result, element, _target):
    if _target == "contract":
        target = smartfast.get_contract_from_name(element["name"])

    elif _target == "structure":
        target = _get_from_contract(smartfast, element, element["name"], "get_structure_from_name")

    elif _target == "event":
        target = _get_from_contract(smartfast, element, element["name"], "get_event_from_name")
        ...
        if element["name"] != element["type_specific_fields"]["parent"]["name"]:
            ...
            target = _get_from_contract(
                smartfast, element, function_sig, "get_function_from_signature"
            )

    elif _target == "modifier":
        modifier_sig = element["type_specific_fields"]["signature"]
        target = _get_from_contract(smartfast, element, modifier_sig, "get_modifier_from_signature")
        ...
        # State variable
            ...
            target = _get_from_contract(
                smartfast, element, element["name"], "get_state_variable_from_name"
            )

    elif _target == "enum":
        target = _get_from_contract(
            smartfast, element, element["name"], "get_enum_from_canonical_name"
        )

    else:
        raise FormatError("Unknown naming convention! " + _target)

    _explore(smartfast, result, target, conventions[element["additional_fields"]["convention"]])
...
def _is_var_declaration(smartfast, filename, start):
    """
    ...
    """
    v = "var "
    return smartfast.source_code[filename][start : start + len(v)] == v


def _explore_type(  # pylint: disable=too-many-arguments,too-many-locals,too-many-branches
    smartfast, result, target, convert, custom_type, filename_source_code, start, end
):
    if isinstance(custom_type, UserDefinedType):
        ...
        if isinstance(custom_type.type, (Enum, Contract)):
            if custom_type.type == target:
                ...
                new_str = convert(old_str, smartfast)

                loc_start = start
                if _is_var_declaration(smartfast, filename_source_code, start):
            ...
            _explore_variables_declaration(
                smartfast, custom_type.type.elems.values(), result, target, convert
            )
    ...
    if isinstance(custom_type, MappingType):
        ...
        if isinstance(custom_type.type_to, (UserDefinedType, MappingType)) or target in [
            ...
            full_txt_end = end
            full_txt = smartfast.source_code[filename_source_code].encode("utf8")[
...
def _explore_variables_declaration(  # pylint: disable=too-many-arguments,too-many-locals,too-many-nested-blocks
    smartfast, variables, result, target, convert, patch_comment=False
):
            ...
            if patch_comment and isinstance(variable, LocalVariable):
                if "lines" in variable.source_mapping and variable.source_mapping["lines"]:
                    ...
                    if variable in func.parameters:
                        ...
                        potential_comments = smartfast.source_code[filename_source_code].encode(
                            "utf8"
                        )
...
def _explore_structures_declaration(smartfast, structures, result, target, convert):
    for st in structures:
        # Explore the variable declared within the structure (VariableStructure)
        _explore_variables_declaration(smartfast, st.elems.values(), result, target, convert)
...
def _explore_events_declaration(smartfast, events, result, target, convert):
    for event in events:
        # Explore the parameters
        _explore_variables_declaration(smartfast, event.elems, result, target, convert)
...
def _explore_irs(smartfast, irs, result, target, convert):
...
def _explore_functions(smartfast, functions, result, target, convert):
    for function in functions:
        _explore_variables_declaration(smartfast, function.variables, result, target, convert, True)
        _explore_irs(smartfast, function.all_smartir_operations(), result, target, convert)
...
def _explore_enums(smartfast, enums, result, target, convert):
    for enum in enums:
        if enum == target:
            old_str = enum.name
            new_str = convert(old_str, smartfast)
...
def _explore_contract(smartfast, contract, result, target, convert):
    _explore_variables_declaration(smartfast, contract.state_variables, result, target, convert)
    _explore_structures_declaration(smartfast, contract.structures, result, target, convert)
    _explore_functions(smartfast, contract.functions_and_modifiers, result, target, convert)
    _explore_enums(smartfast, contract.enums, result, target, convert)
...
def _explore(smartfast, result, target, convert):
    for contract in smartfast.contracts_derived:
        _explore_contract(smartfast, contract, result, target, convert)
