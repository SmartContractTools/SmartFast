...
class PrinterCallGraph(AbstractPrinter):
    ...
    WIKI = "https://github.com/trailofbits/smartfast/wiki/Printer-documentation#call-graph"
    ...
    def output(self, filename):
        ...
        with open(all_contracts_filename, "w", encoding="utf8") as f:
            ...
            content = "\n".join(
                ["strict digraph {"] + [_process_functions(self.smartfast.functions)] + ["}"]
            )
            f.write(content)
            results.append((all_contracts_filename, content))

        for derived_contract in self.smartfast.contracts_derived:
