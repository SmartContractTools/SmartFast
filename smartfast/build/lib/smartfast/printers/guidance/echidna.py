...
from smartfast.core.expressions import NewContract
...
def _extract_payable(smartfast: SmartfastCore) -> Dict[str, List[str]]:
...
def _extract_solidity_variable_usage(
    smartfast: SmartfastCore, sol_var: SolidityVariable
) -> Dict[str, List[str]]:
    ret: Dict[str, List[str]] = {}
    for contract in smartfast.contracts:
...
def _is_constant(f: Function) -> bool:  # pylint: disable=too-many-branches
    ...
    - If it contains assembly -> Return false (SmartfastCore doesn't analyze asm)
    ...
    """
    if f.view or f.pure:
        if f.contract.smartfast.crytic_compile and f.contract.smartfast.crytic_compile.compiler_version:
            if not f.contract.smartfast.crytic_compile.compiler_version.version.startswith("0.4"):
                return True
    if f.payable:
        return False
    ...
    if f.all_state_variables_written():
        return False
    for ir in f.all_smartir_operations():
        ...
        if isinstance(ir, HighLevelCall):
            if isinstance(ir.function, Variable) or ir.function.view or ir.function.pure:
                ...
                if (
                    f.contract.smartfast.crytic_compile
                    and f.contract.smartfast.crytic_compile.compiler_version
                ):
                    if f.contract.smartfast.crytic_compile.compiler_version.version.startswith("0.4"):
                        return False
            else:
                return False
...
def _extract_constant_functions(smartfast: SmartfastCore) -> Dict[str, List[str]]:
    ret: Dict[str, List[str]] = {}
    for contract in smartfast.contracts:
...
def _extract_assert(smartfast: SmartfastCore) -> Dict[str, List[str]]:
    ret: Dict[str, List[str]] = {}
    for contract in smartfast.contracts:
...
def _extract_constants(
    ...
    for contract in smartfast.contracts:
        for function in contract.functions_entry_points:
            ...
            _extract_constants_from_irs(
                function.all_smartir_operations(),
                all_cst_used,
                all_cst_used_in_binary,
                context_explored,
            )
...
def _have_external_calls(smartfast: SmartfastCore) -> Dict[str, List[str]]:
    """
...
def _use_balance(smartfast: SmartfastCore) -> Dict[str, List[str]]:
    """
    Detect the functions with external calls
    :param smartfast:
    :return:
    ...
    for contract in smartfast.contracts:
        for function in contract.functions_entry_points:
            for ir in function.all_smartir_operations():
...
def _call_a_parameter(smartfast: SmartfastCore) -> Dict[str, List[Dict]]:
    """
    Detect the functions with external calls
    :param smartfast:
    :return:
    ...
    for contract in smartfast.contracts:  # pylint: disable=too-many-nested-blocks
        for function in contract.functions_entry_points:
            for ir in function.all_smartir_operations():
                if isinstance(ir, HighLevelCall):
...
class Echidna(AbstractPrinter):
    ...
    WIKI = "https://github.com/trailofbits/smartfast/wiki/Printer-documentation#echidna"
    ...
    def output(self, filename):  # pylint: disable=too-many-locals
        ...
        payable = _extract_payable(self.smartfast)
        timestamp = _extract_solidity_variable_usage(
            self.smartfast, SolidityVariableComposed("block.timestamp")
        )
        block_number = _extract_solidity_variable_usage(
            self.smartfast, SolidityVariableComposed("block.number")
        )
        msg_sender = _extract_solidity_variable_usage(
            self.smartfast, SolidityVariableComposed("msg.sender")
        )
        msg_gas = _extract_solidity_variable_usage(
            self.smartfast, SolidityVariableComposed("msg.gas")
        )
        assert_usage = _extract_assert(self.smartfast)
        cst_functions = _extract_constant_functions(self.smartfast)
        (cst_used, cst_used_in_binary) = _extract_constants(self.smartfast)

        functions_relations = _extract_function_relations(self.smartfast)
        ...
        external_calls = _have_external_calls(self.smartfast)

        call_parameters = _call_a_parameter(self.smartfast)

        use_balance = _use_balance(self.smartfast)
