...
from smartfast.utils.inheritance_analysis import (
...
class PrinterInheritanceGraph(AbstractPrinter):
    ...
    WIKI = "https://github.com/trailofbits/smartfast/wiki/Printer-documentation#inheritance-graph"

    def __init__(self, smartfast, logger):
        super().__init__(smartfast, logger)

        inheritance = [x.inheritance for x in smartfast.contracts]
        self.inheritance = {item for sublist in inheritance for item in sublist}

        self.overshadowing_state_variables = {}
        shadows = detect_state_variable_shadowing(smartfast.contracts)
