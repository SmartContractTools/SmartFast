...
from smartfast.core.expressions import Identifier, Literal
...
from smartfast.core.solidity_types.elementary_type import Int as ElementaryTypeInt
...
from smartfast.smartir.operations.codesize import CodeSize
from smartfast.smartir.tmp_operations.argument import Argument, ArgumentType
from smartfast.smartir.tmp_operations.tmp_call import TmpCall
from smartfast.smartir.tmp_operations.tmp_new_array import TmpNewArray
from smartfast.smartir.tmp_operations.tmp_new_contract import TmpNewContract
from smartfast.smartir.tmp_operations.tmp_new_elementary_type import TmpNewElementaryType
from smartfast.smartir.tmp_operations.tmp_new_structure import TmpNewStructure
from smartfast.smartir.variables import Constant, ReferenceVariable, TemporaryVariable
from smartfast.smartir.variables import TupleVariable
from smartfast.utils.function import get_function_id
from smartfast.utils.type import export_nested_types_from_variable
from smartfast.visitors.smartir.expression_to_smartir import ExpressionToSmartIR
...
def convert_expression(expression, node):
    ...
    visitor = ExpressionToSmartIR(expression, node)
...
def _convert_type_contract(ir, smartfast):
    ...
    if ir.variable_right == "creationCode":
        if smartfast.crytic_compile:
            bytecode = smartfast.crytic_compile.bytecode_init(contract.name)
        else:
            logger.info(
    ...
    if ir.variable_right == "runtimeCode":
        if smartfast.crytic_compile:
            bytecode = smartfast.crytic_compile.bytecode_runtime(contract.name)
        else:
            logger.info(
    ...
    raise SmartIRError(f"type({contract.name}).{ir.variable_right} is unknown")
            ...
            if isinstance(ir, Assignment):
                ...
                if isinstance(t, UserDefinedType):
                    ...
                    if isinstance(t_type, Contract):
                        contract = node.smartfast.get_contract_from_name(t_type.name)
                ...
                if isinstance(ir.variable_left, TemporaryVariable) and isinstance(
                    ...
                    return _convert_type_contract(ir, node.function.smartfast)
                ...
                contract = node.smartfast.get_contract_from_name(ir.contract_name)
                ...
                raise SmartIRError("Not handling {} during type propagation".format(type(ir)))
    return None
...
def extract_tmp_call(ins, contract):  # pylint: disable=too-many-locals
    ...
    if isinstance(ins.ori, Member):
        ...
        if isinstance(ins.ori.variable_left, Function):
            ...
            #     }
            # }
            to_log = "Smartfast does not support dynamic functions to libraries if functions have the same name"
            to_log += f"{[candidate.full_name for candidate in candidates]}"
            raise SmartIRError(to_log)
...
def convert_to_low_level(ir):
    ...
    if ir.function_name in ["call", "delegatecall", "callcode", "staticcall"]:
        ...
        if ir.smartfast.solc_version >= "0.5":
        ...
        return new_ir
    raise SmartIRError("Incorrect conversion to low level {}".format(ir))
...
def convert_to_solidity_func(ir):
    ...
    # abi.decode(a, (uint))
        ...
        # See https://github.com/SmartContractTools/SmartFast/issues/566 for potential solutions
...
def look_for_library(contract, ir, using_for, t):
    for destination in using_for[t]:
        lib_contract = contract.smartfast.get_contract_from_name(str(destination))
        if lib_contract:
            lib_call = LibraryCall(
...
def apply_ir_heuristics(irs, node):
    """
    Apply a set of heuristic to improve smartIR
    """
