...
from smartfast.core.cfg.node import NodeType, link_nodes, insert_node, Node
from smartfast.core.declarations.contract import Contract
from smartfast.core.declarations.function import (
    Function,
    ModifierStatements,
    FunctionType,
)
...
from smartfast.core.expressions import AssignmentOperation
...
from smartfast.solc_parsing.cfg.node import NodeSolc
from smartfast.solc_parsing.expressions.expression_parsing import parse_expression
from smartfast.solc_parsing.variables.local_variable import LocalVariableSolc
from smartfast.solc_parsing.variables.local_variable_init_from_tuple import (
    LocalVariableInitFromTupleSolc,
)
from smartfast.solc_parsing.variables.variable_declaration import MultipleVariablesDeclaration
from smartfast.solc_parsing.yul.parse_yul import YulBlock
from smartfast.utils.expression_manipulations import SplitTernaryExpression
from smartfast.visitors.expression.export_values import ExportValues
from smartfast.visitors.expression.has_conditional import HasConditional
...
class FunctionSolc:
    ...
    def __init__(
        ...
        smartfast_parser: "SmartfastSolc",
    ):
        self._smartfast_parser: "SmartfastSolc" = smartfast_parser
    ...
    def smartfast(self) -> "SmartfastCore":
        return self._function.smartfast
    ...
    def _parse_try_catch(self, statement: Dict, node: NodeSolc) -> NodeSolc:
        ...
        if externalCall is None:
            raise ParsingError("Try/Catch not correctly parsed by Smartfast %s" % statement)
    ...
    def _parse_catch(self, statement: Dict, node: NodeSolc) -> NodeSolc:
        ...
        if block is None:
            raise ParsingError("Catch not correctly parsed by Smartfast %s" % statement)
    ...
    def _parse_variable_definition(self, statement: Dict, node: NodeSolc) -> NodeSolc:
        try:
            ...
            local_var.set_offset(statement["src"], self._function.smartfast)
    ...
    def _parse_statement(self, statement: Dict, node: NodeSolc) -> NodeSolc:
        ...
        if name == "IfStatement":
            ...
            if "AST" in statement and not self.smartfast.skip_assembly:
    ...
    def _add_param(self, param: Dict) -> LocalVariableSolc:
        ...
        local_var = LocalVariable()
        local_var.set_function(self._function)
        local_var.set_offset(param["src"], self._function.smartfast)
    ...
    def _parse_params(self, params: Dict):
        ...
        self.parameters_src.set_offset(params["src"], self._function.smartfast)
    ...
    def _parse_returns(self, returns: Dict):
        ...
        self.returns_src.set_offset(returns["src"], self._function.smartfast)
