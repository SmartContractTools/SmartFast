...
from smartfast.core.declarations import SolidityFunction, Enum
from smartfast.core.declarations.contract import Contract
from smartfast.core.declarations.structure import Structure
from smartfast.core.solidity_types import MappingType, ArrayType
...
from smartfast.smartir.operations import NewContract, TypeConversion, SolidityCall
from smartfast.tools.flattening.export.export import (
    Export,
    export_as_json,
    save_to_zip,
    save_to_disk,
...
logger = logging.getLogger("Smartfast-flattening")
...
class Flattening:
    ...
    def _check_abi_encoder_v2(self):
        ...
        :return:
        """
        for p in self._smartfast.pragma_directives:
    ...
    def _get_source_code(
        ...
        content = self._smartfast.source_code[src_mapping["filename_absolute"]].encode("utf8")
            ...
            for f in contract.functions_declared:
                ...
                if f.visibility == "external":
                    ...
                    if regex:
                        ...
                        raise SmartfastException(f"External keyword not found {f.name} {attributes}")
    ...
    def _pragmas(self) -> str:
        ...
        elif self._smartfast.solc_version:
            ret += f"pragma solidity {self._smartfast.solc_version};\n"
    ...
    def _export_list_used_contracts(  # pylint: disable=too-many-branches
        ...
        for f in contract.functions_declared:
            for ir in f.smartir_operations:
    ...
    def _export_most_derived(self) -> List[Export]:
        ret: List[Export] = []
        for contract in self._smartfast.contracts_derived:
    ...
    def _export_all(self) -> List[Export]:
        ...
        contract_seen = set()
        contract_to_explore = list(self._smartfast.contracts)
    ...
    def _export_with_import(self) -> List[Export]:
        exports: List[Export] = []
        for contract in self._smartfast.contracts:
            list_contracts: List[Contract] = []  # will contain contract itself
            self._export_list_used_contracts(contract, set(), list_contracts)
    ...
    def export(  # pylint: disable=too-many-arguments,too-few-public-methods
        ...
        if target is None:
            ...
            contract = self._smartfast.get_contract_from_name(target)
            if contract is None:
