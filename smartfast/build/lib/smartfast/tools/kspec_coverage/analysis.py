...
from smartfast.utils.colors import yellow, green, red
from smartfast.utils import output

logging.basicConfig(level=logging.WARNING)
logger = logging.getLogger("Smartfast.kspec")
...
def _get_smartfast_functions(smartfast):
    ...
    all_functions_declared += list(
        {s for s in smartfast.state_variables if s.visibility in ["public", "external"]}
    )
    smartfast_functions = {
...
def _run_coverage_analysis(args, smartfast, kspec_functions):
    # Collect all smartfast functions
    smartfast_functions = _get_smartfast_functions(smartfast)

    # Determine which klab specs were not resolved.
    smartfast_functions_set = set(smartfast_functions)
    kspec_functions_resolved = kspec_functions & smartfast_functions_set
    kspec_functions_unresolved = kspec_functions - kspec_functions_resolved

    kspec_missing = []
    kspec_present = []

    for smartfast_func_desc in sorted(smartfast_functions_set):
        smartfast_func = smartfast_functions[smartfast_func_desc]

        if smartfast_func_desc in kspec_functions:
            kspec_present.append(smartfast_func)
        else:
            kspec_missing.append(smartfast_func)
...
def run_analysis(args, smartfast, kspec_arg):
    ...
    _run_coverage_analysis(args, smartfast, kspec_functions)
