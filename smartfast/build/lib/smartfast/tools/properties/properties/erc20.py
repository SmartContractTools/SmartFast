...
from smartfast.tools.properties.addresses.address import Addresses
from smartfast.tools.properties.platforms.echidna import generate_echidna_config
from smartfast.tools.properties.properties.ercs.erc20.properties.burn import ERC20_NotBurnable
from smartfast.tools.properties.properties.ercs.erc20.properties.initialization import ERC20_CONFIG
from smartfast.tools.properties.properties.ercs.erc20.properties.mint import ERC20_NotMintable
from smartfast.tools.properties.properties.ercs.erc20.properties.mint_and_burn import (
    ERC20_NotMintableNotBurnable,
)
from smartfast.tools.properties.properties.ercs.erc20.properties.transfer import (
    ERC20_Transferable,
    ERC20_Pausable,
)
from smartfast.tools.properties.properties.ercs.erc20.unit_tests.truffle import generate_truffle_test
from smartfast.tools.properties.properties.properties import (
    property_to_solidity,
    Property,
)
from smartfast.tools.properties.solidity.generate_properties import (
    generate_solidity_properties,
    generate_test_contract,
    generate_solidity_interface,
)
from smartfast.utils.colors import red, green
...
def generate_erc20(
    ...
    :return:
    """
    if contract.smartfast.crytic_compile is None:
        logging.error("Please compile with crytic-compile")
        return
    if contract.smartfast.crytic_compile.type not in [
        PlatformType.TRUFFLE,
        PlatformType.SOLC,
    ]:
        logging.error(f"{contract.smartfast.crytic_compile.type} not yet supported by smartfast-prop")
        return
    ...
    output_dir = _platform_to_output_dir(contract.smartfast.crytic_compile.platform)
    ...
    echidna_config_filename = generate_echidna_config(
        Path(contract.smartfast.crytic_compile.target).parent, addresses
    )

    unit_test_info = ""

    # If truffle, generate unit tests
    if contract.smartfast.crytic_compile.type == PlatformType.TRUFFLE:
    ...
    txt = f"\t echidna-test {contract.smartfast.crytic_compile.target} "
...
def _check_compatibility(contract):
    ...
    if not contract.is_erc20():
        errors = f"{contract} is not ERC20 compliant. Consider checking the contract with smartfast-check-erc"
        return errors

    transfer = contract.get_function_from_signature("transfer(address,uint256)")

    if transfer.visibility != "public":
        errors = f"smartfast-prop requires {transfer.canonical_name} to be public. Please change the visibility"
    ...
    if transfer_from.visibility != "public":
        if errors:
            errors += "\n"
        errors += f"smartfast-prop requires {transfer_from.canonical_name} to be public. Please change the visibility"

    approve = contract.get_function_from_signature("approve(address,uint256)")
    if approve.visibility != "public":
        if errors:
            errors += "\n"
        errors += f"smartfast-prop requires {approve.canonical_name} to be public. Please change the visibility"
