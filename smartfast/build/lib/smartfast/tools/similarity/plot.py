...
try:
    ...
    print("ERROR: in order to use smartfast-simil, you need to install numpy:")
    print("$ pip3 install numpy --user\n")
    sys.exit(-1)

from smartfast.tools.similarity.encode import load_and_encode, parse_target
from smartfast.tools.similarity.model import load_model
...
def plot(args):  # pylint: disable=too-many-locals
    ...
    if decomposition is None or plt is None:
        logger.error(
            "ERROR: In order to use plot mode in smartfast-simil, you need to install sklearn and matplotlib:"
        )
