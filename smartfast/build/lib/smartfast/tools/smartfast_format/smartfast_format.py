...
from smartfast.detectors.variables.unused_state_variables import UnusedStateVars
from smartfast.detectors.attributes.incorrect_solc import IncorrectSolc
from smartfast.detectors.attributes.constant_pragma import ConstantPragma
from smartfast.detectors.naming_convention.naming_convention import NamingConvention
from smartfast.detectors.functions.external_function import ExternalFunction
from smartfast.detectors.variables.possible_const_state_variables import ConstCandidateStateVars
from smartfast.detectors.attributes.const_functions_asm import ConstantFunctionsAsm
from smartfast.detectors.attributes.const_functions_state import ConstantFunctionsState
from smartfast.utils.colors import yellow
...
def smartfast_format(smartfast, **kwargs):  # pylint: disable=too-many-locals
    """'
    Keyword Args:
    ...
    for detector in detectors_to_run:
        smartfast.register_detector(detector)

    smartfast.generate_patches = True
    ...
    logger.info(yellow("smartfast-format is in beta, carefully review each patch before merging it."))
...
def print_patches(number_of_smartfast_results, patches):
    logger.info("Number of Smartfast results: " + str(number_of_smartfast_results))
    number_of_patches = 0
    for file in patches:
...
def print_patches_json(number_of_smartfast_results, patches):
    print("{", end="")
    print('"Number of Smartfast results":' + '"' + str(number_of_smartfast_results) + '",')
