...
from smartfast.utils.output import output_to_json
from smartfast.tools.upgradeability.checks import all_checks
from smartfast.tools.upgradeability.checks.abstract_checks import AbstractCheck
from smartfast.tools.upgradeability.utils.command_line import (
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Smartfast Upgradeability Checks. For usage information see https://github.com/SmartContractTools/SmartFast/wiki/Upgradeability-Checks.",
        usage="smartfast-check-upgradeability contract.sol ContractName",
    )
...
def main():
    ...
    try:
        variable1 = Smartfast(v1_filename, **vars(args))
        ...
        if args.proxy_name:
            if args.proxy_filename:
                proxy = Smartfast(args.proxy_filename, **vars(args))
            else:
                proxy = variable1
        ...
        if args.new_contract_name:
            if args.new_contract_filename:
                variable2 = Smartfast(args.new_contract_filename, **vars(args))
            else:
                variable2 = variable1
        ...
        logger.error(str(smartfast_exception))
        if args.json:
            output_to_json(args.json, str(smartfast_exception), json_results)
        return
