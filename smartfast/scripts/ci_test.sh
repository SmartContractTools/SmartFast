...
CURRENT_PATH=$(pwd)
TRAVIS_PATH='/home/travis/build/crytic/smartfast'

# test_smartfast file.sol detectors
test_smartfast(){

    expected="$DIR/../tests/expected_json/$(basename "$1" .sol).$2.json"

    # run smartfast detector on input file and save output as json
    smartfast "$1" --solc-disable-warnings --detect "$2" --json "$DIR/tmp-test.json" 
    if [ $? -eq 255 ]
    then
        echo "Smartfast crashed"
        exit 255
    fi
    ...
    # run smartfast detector on input file and save output as json
    smartfast "$1" --solc-disable-warnings --detect "$2" --legacy-ast --json "$DIR/tmp-test.json" 
    if [ $? -eq 255 ]
    then
        echo "Smartfast crashed"
        exit 255
...
generate_expected_json(){
    ...
    # run smartfast detector on input file and save output as json
    smartfast "$1" --solc-disable-warnings --detect "$2" --json "$output_filename" > "$output_filename_txt" 2>&1
