import logging
from smartfast.core.declarations import Contract, Function
...
from smartfast.analyses.evm.evm_cfg_builder import load_evm_cfg_builder
...
def get_evm_instructions(obj):
    ...
    if KEY_EVM_INS not in obj.context:
        ...
        smartfast = obj.smartfast

        if not smartfast.crytic_compile:
            raise SmartfastError("EVM features require to compile with crytic-compile")
        ...
        contract_info["bytecode_runtime"] = smartfast.crytic_compile.bytecode_runtime(
            contract_info["contract"].name
        )
        contract_info["srcmap_runtime"] = smartfast.crytic_compile.srcmap_runtime(
            contract_info["contract"].name
        )
        contract_info["cfg"] = CFG(contract_info["bytecode_runtime"])

        # Get contract init bytecode, srcmap and cfg
        contract_info["bytecode_init"] = smartfast.crytic_compile.bytecode_init(
            contract_info["contract"].name
        )
        contract_info["srcmap_init"] = smartfast.crytic_compile.srcmap_init(
            contract_info["contract"].name
        )
...
def _get_evm_instructions_function(function_info):
    ...
    if function_evm is None:
        ...
        logger.error(to_log)
        raise SmartfastError("Function " + function.name + " not found in the EVM code")
...
def generate_source_to_evm_ins_mapping(evm_instructions, srcmap_runtime, smartfast, filename):
    """
    ...
    file_source = smartfast.source_code[filename].encode("utf-8")
    prev_mapping = []
