...
from smartfast.core.children.child_function import ChildFunction
...
from smartfast.all_exceptions import SmartfastException
...
class Node(SourceMapping, ChildFunction):  # pylint: disable=too-many-public-methods
    ...
    def __init__(self, node_type: NodeType, node_id: int):
        ...
        self._ssa_vars_written: List["SmartIRVariable"] = []
        self._ssa_vars_read: List["SmartIRVariable"] = []
        ...
        self._smartir_vars: Set["SmartIRVariable"] = set()  # non SSA
    ...
    def smartfast(self) -> "SmartfastCore":
        return self.function.smartfast
    ...
    @property
    def ssa_variables_read(self) -> List["SmartIRVariable"]:
        """
        list(Variable): Variables read (local/state/solidity)
        """
    ...
    def smartir_variables(self) -> List["SmartIRVariable"]:
        return list(self._smartir_vars)
    ...
    @property
    def ssa_variables_written(self) -> List["SmartIRVariable"]:
        """
        list(Variable): Variables written (local/state/solidity)
        """
    ...
    def irs(self) -> List[Operation]:
        """Returns the smartIR representation

        return
            list(smartIR.Operation)
        """
        return self._irs

    @property
    def irs_ssa(self) -> List[Operation]:
        """Returns the smartIR representation with SSA
    ...
    def all_smartir_operations(self) -> List[Operation]:
        ...
        for ir in irs:
            if isinstance(ir, InternalCall):
                irs += ir.function.all_smartir_operations()
        return irs

    @staticmethod
    def _is_non_smartir_var(var: Variable):
        return not isinstance(var, (Constant, ReferenceVariable, TemporaryVariable, TupleVariable))

    @staticmethod
    def _is_valid_smartir_var(var: Variable):
    ...
    def _find_read_write_call(self):  # pylint: disable=too-many-statements
        ...
        for ir in self.irs:
            ...
            self._smartir_vars |= {v for v in ir.read if self._is_valid_smartir_var(v)}

            if isinstance(ir, OperationWithLValue):
                var = ir.lvalue
                if var and self._is_valid_smartir_var(var):
                    self._smartir_vars.add(var)

            if not isinstance(ir, (Phi, Index, Member)):
                self._vars_read += [v for v in ir.read if self._is_non_smartir_var(v)]
                for var in ir.read:
                ...
                if self._is_non_smartir_var(var):
                    self._vars_read.append(var)
                if isinstance(var, ReferenceVariable):
                    origin = var.points_to_origin
                    if self._is_non_smartir_var(origin):
            ...
            if isinstance(ir, OperationWithLValue):
                ...
                if var and self._is_non_smartir_var(var):
    ...
    def update_read_write_using_ssa(self):
        ...
        for ir in self.irs_ssa:
            ...
            if isinstance(ir, OperationWithLValue):
                ...
                if isinstance(var, ReferenceVariable):
                    var = var.points_to_origin
                # Only store non-smartIR variables
