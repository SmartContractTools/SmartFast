...
from smartfast.core.children.child_smartfast import ChildSmartfast
from smartfast.core.solidity_types.type import Type
from smartfast.core.source_mapping.source_mapping import SourceMapping

from smartfast.core.declarations.function import Function, FunctionType
from smartfast.utils.erc import (
    ERC20_signatures,
    ERC165_signatures,
    ERC223_signatures,
    ERC721_signatures,
...
from smartfast.utils.tests_pattern import is_test_contract

# pylint: disable=too-many-lines,too-many-instance-attributes,import-outside-toplevel,too-many-nested-blocks
if TYPE_CHECKING:
    from smartfast.utils.type_helpers import LibraryCallType, HighLevelCallType, InternalCallType
    from smartfast.core.declarations import Enum, Event, Modifier, EnumContract, StructureContract, Structure
    from smartfast.smartir.variables.variable import SmartIRVariable
...
class Contract(ChildSmartfast, SourceMapping):  # pylint: disable=too-many-public-methods
    """
    Contract class
    """
    ...
    @property
    def smartir_variables(self) -> List["SmartIRVariable"]:
        """
        List all of the smartir variables (non SSA)
        """
        smartir_variabless = [f.smartir_variables for f in self.functions + self.modifiers]  # type: ignore
        smartir_variables = [item for sublist in smartir_variabless for item in sublist]
        return list(set(smartir_variables))
    ...
    def derived_contracts(self) -> List["Contract"]:
        ...
        """
        candidates = self.smartfast.contracts
    ...
    @property
    def all_functions_called_notshadowed(self) -> List["Function"]:
        """
            list(Function): List of functions reachable from the contract
            Includes super, and private/internal functions not shadowed
        """
        all_calls = [f for f in self.functions + self.modifiers]  # type: ignore
        all_callss = [f.all_internal_calls() for f in all_calls] + [list(all_calls)]
        all_calls = [item for sublist in all_callss for item in sublist]
        all_calls = list(set(all_calls))
    ...
    def ercs(self) -> List[str]:
        ...
        all_erc = [
            ...
            ("ERC621", lambda x: x.is_erc621()),
            ("ERC875", lambda x: x.is_erc875()),
            ("ERC1155", lambda x: x.is_erc1155()),
            ("ERC1190", lambda x: x.is_erc1190()),
            ("ERC1410", lambda x: x.is_erc1410()),
        ]
    ...
    def is_erc621(self) -> bool:
        """
            Check if the contract is an erc621

            Note: it does not check for correct return values
        :return: Returns a true if the contract is an erc165
        """
        full_names = self.functions_signatures
        return all((s in full_names for s in ERC621_signatures))

    def is_erc875(self) -> bool:
        """
            Check if the contract is an erc875

            Note: it does not check for correct return values
        :return: Returns a true if the contract is an erc165
        """
        full_names = self.functions_signatures
        return all((s in full_names for s in ERC875_signatures))
    ...
    def is_erc1190(self) -> bool:
        """
            Check if the contract is an erc1190

            Note: it does not check for correct return values
        :return: Returns a true if the contract is an erc165
        """
        full_names = self.functions_signatures
        return all((s in full_names for s in ERC1190_signatures))

    def is_erc1410(self) -> bool:
        """
            Check if the contract is an erc1410

            Note: it does not check for correct return values
        :return: Returns a true if the contract is an erc165
        """
        full_names = self.functions_signatures
        return all((s in full_names for s in ERC1410_signatures))
    ...
    def is_possible_erc1410(self) -> bool:
        """
        Checks if the provided contract could be attempting to implement ERC1410 standards.

        :return: Returns a boolean indicating if the provided contract met the token standard.
        """
        full_names = self.functions_signatures
        return (
            "balanceOfByPartition(bytes32,address)" in full_names
            or "partitionsOf(address)" in full_names
            or "transferByPartition(bytes32,address,uint256,bytes)" in full_names
            or "operatorTransferByPartition(bytes32,address,address,uint256,bytes,bytes)" in full_names
            or "canTransferByPartition(address,address,bytes32,uint256,bytes)" in full_names
            or "isOperator(address,address)" in full_names
            or "isOperatorForPartition(bytes32,address,address)" in full_names
            or "authorizeOperatorByPartition(bytes32,address)" in full_names
            or "revokeOperatorByPartition(bytes32,address)" in full_names
            or "issueByPartition(bytes32,address,uint256,bytes)" in full_names
            or "redeemByPartition(bytes32,uint256,bytes)" in full_names
            or "operatorRedeemByPartition(bytes32,address,uint256,bytes)" in full_names
        )

    def is_possible_erc1190(self) -> bool:
        """
        Checks if the provided contract could be attempting to implement ERC1190 standards.

        :return: Returns a boolean indicating if the provided contract met the token standard.
        """
        full_names = self.functions_signatures
        return (
            "transferCreativeLicense(address[],address[],uint256)" in full_names
            or "transferOwnershipLicense(address[],address[],address[],uint256)" in full_names
            or "rentAsset(address[],address[],address[],uint256)" in full_names
        )

    def is_possible_erc1155(self) -> bool:
        """
        Checks if the provided contract could be attempting to implement ERC1155 standards.

        :return: Returns a boolean indicating if the provided contract met the token standard.
        """
        full_names = self.functions_signatures
        return (
            "safeBatchTransferFrom(address,address,uint256[],uint256[],bytes)" in full_names
            or "safeTransferFrom(address,address,uint256,uint256,bytes)" in full_names
            or "balanceOfBatch(address[],address[],address[],uint256)" in full_names
        )

    def is_possible_erc875(self) -> bool:
        """
        Checks if the provided contract could be attempting to implement ERC875 standards.

        :return: Returns a boolean indicating if the provided contract met the token standard.
        """
        full_names = self.functions_signatures
        return (
            "transferFrom(address,address,uint256[])" in full_names
            or "transfer(address,uint256[])" in full_names
        )

    def is_possible_erc621(self) -> bool:
        """
        Checks if the provided contract could be attempting to implement ERC621 standards.

        :return: Returns a boolean indicating if the provided contract met the token standard.
        """
        full_names = self.functions_signatures
        return (
            "increaseSupply(uint256,address)" in full_names
            or "decreaseSupply(uint256,address)" in full_names
        )

    def is_possible_erc777(self) -> bool:
        """
        Checks if the provided contract could be attempting to implement ERC777 standards.

        :return: Returns a boolean indicating if the provided contract met the token standard.
        """
        full_names = self.functions_signatures
        return (
            "granularity()" in full_names
            or "isOperatorFor(address,address)" in full_names
            or "operatorSend(address,address,uint256,bytes,bytes)" in full_names
            or "operatorBurn(address,uint256,bytes,bytes)" in full_names
        )

    def is_possible_erc223(self) -> bool:
        """
        Checks if the provided contract could be attempting to implement ERC223 standards.

        :return: Returns a boolean indicating if the provided contract met the token standard.
        """
        full_names = self.functions_signatures
        return (
            "transfer(address,uint256,bytes,string)" in full_names
        )
    ...
    def is_possible_token(self) -> bool:
        ...
        :return:
        """
        return self.is_possible_erc20() or self.is_possible_erc721() or self.is_possible_erc1410() or self.is_possible_erc1190() or self.is_possible_erc1155() or self.is_possible_erc875() or self.is_possible_erc621() or self.is_possible_erc777() or self.is_possible_erc223()
    ...
    def is_from_dependency(self) -> bool:
        if self.smartfast.crytic_compile is None:
            return False
        return self.smartfast.crytic_compile.is_dependency(self.source_mapping["filename_absolute"])
    ...
    def is_truffle_migration(self) -> bool:
        ...
        if self.smartfast.crytic_compile:
            if self.smartfast.crytic_compile.platform == PlatformType.TRUFFLE:
    ...
    def is_upgradeable(self) -> bool:
        if self._is_upgradeable is None:
            ...
            if self.is_upgradeable_proxy:
                return False
            initializable = self.smartfast.get_contract_from_name("Initializable")
            if initializable:
    ...
    def is_incorrectly_constructed(self) -> bool:
        """
        Return true if there was an internal Smartfast's issue when analyzing the contract
        :return:
        """
        ...
        if self.state_variables:
            for (idx, variable_candidate) in enumerate(self.state_variables):
                if variable_candidate.expression and not variable_candidate.is_constant:
                    ...
                    constructor_variable = FunctionContract(self.smartfast)
                    ...
                    constructor_variable.set_offset(self.source_mapping, self.smartfast)
    ...
    def _create_node(self, func: Function, counter: int, variable: "Variable"):
        from smartfast.core.cfg.node import Node, NodeType
        from smartfast.core.expressions import (
        ...
        node.set_offset(variable.source_mapping, self.smartfast)
        ...
        expression.set_offset(variable.source_mapping, self.smartfast)
    ...
    def convert_expression_to_smartir_ssa(self):
        """
        Assume generate_smartir_and_analyze was called on all functions

        :return:
        """
        from smartfast.smartir.variables import StateIRVariable
        ...
        for func in self.functions + self.modifiers:
            func.generate_smartir_ssa(all_ssa_state_variables_instances)
