...
from smartfast.core.declarations.solidity_variables import (
    SolidityFunction,
    SolidityVariable,
...
from smartfast.core.solidity_types import UserDefinedType
...
if TYPE_CHECKING:
    from smartfast.utils.type_helpers import (
        InternalCallType,
        LowLevelCallType,
        HighLevelCallType,
        LibraryCallType,
    ...
    from smartfast.smartir.variables import LocalIRVariable
    from smartfast.core.expressions.expression import Expression
    from smartfast.smartir.operations import Operation
...
class Function(metaclass=ABCMeta):  # pylint: disable=too-many-public-methods
    ...
    def __init__(self, smartfast: "SmartfastCore"):
        super().__init__()
        ...
        # smartir Temporary and references variables (but not SSA)
        self._smartir_variables: Set["SmartIRVariable"] = set()
        ...
        self._return_values: Optional[List["SmartIRVariable"]] = None
        self._return_values_ssa: Optional[List["SmartIRVariable"]] = None
        ...
        self._smartir_operations: Optional[List["Operation"]] = None
        self._smartir_ssa_operations: Optional[List["Operation"]] = None

        self._all_expressions: Optional[List["Expression"]] = None
        self._all_smartir_operations: Optional[List["Operation"]] = None
        ...
        self._all_smartir_variables: Optional[List["SmartIRVariable"]] = None
        ...
        self._smartfast: "SmartfastCore" = smartfast
    ...
    def name(self) -> str:
        ...
        if self._function_type == FunctionType.CONSTRUCTOR_VARIABLES:
            return "smartfastConstructorVariables"
        if self._function_type == FunctionType.CONSTRUCTOR_CONSTANT_VARIABLES:
            return "smartfastConstructorConstantVariables"
        return self._name
    ...
    def can_reenter(self, callstack=None) -> bool:
        ...
        if self._can_reenter is None:
            ...
            for ir in self.all_smartir_operations():
    ...
    @property
    def smartfast(self) -> "SmartfastCore":
        return self._smartfast

    @smartfast.setter
    def smartfast(self, sl: "SmartfastCore"):
        self._smartfast = sl
    ...
    def is_constructor_variables(self) -> bool:
        ...
        Smartfast has inbuilt functions to hold the state variables initialization
        """
    ...
    def smartir_variables(self) -> List["SmartIRVariable"]:
        ...
        return list(self._smartir_variables)
    ...
    @property
    def return_values(self) -> List["SmartIRVariable"]:
        """
        list(Return Values): List of the return values
        """
        from smartfast.core.cfg.node import NodeType
        from smartfast.smartir.operations import Return
    ...
    @property
    def return_values_ssa(self) -> List["SmartIRVariable"]:
        """
    ...
    @property
    def smartir_operations(self) -> List["Operation"]:
        """
        list(Operation): List of the smartir operations
        """
        if self._smartir_operations is None:
            operationss = [n.irs for n in self.nodes]
            operations = [item for sublist in operationss for item in sublist if item]
            self._smartir_operations = operations
        return self._smartir_operations

    @property
    def smartir_ssa_operations(self) -> List["Operation"]:
        """
        list(Operation): List of the smartir operations (SSA)
        """
        if self._smartir_ssa_operations is None:
            operationss = [n.irs_ssa for n in self.nodes]
            operations = [item for sublist in operationss for item in sublist if item]
            self._smartir_ssa_operations = operations
        return self._smartir_ssa_operations
    ...
    def all_smartir_variables(self) -> List["SmartIRVariable"]:
        """recursive version of smartir_variables"""
        if self._all_smartir_variables is None:
            self._all_smartir_variables = self._explore_functions(lambda x: x.smartir_variables)
        return self._all_smartir_variables
    ...
    def all_smartir_operations(self) -> List["Operation"]:
        if self._all_smartir_operations is None:
            self._all_smartir_operations = self._explore_functions(lambda x: x.smartir_operations)
        return self._all_smartir_operations
    ...
    def _solidity_variable_in_binary(node: "Node") -> List[SolidityVariable]:
        from smartfast.smartir.operations.binary import Binary
    ...
    def _solidity_variable_in_internal_calls(node: "Node") -> List[SolidityVariable]:
        from smartfast.smartir.operations.internal_call import InternalCall
    ...
    def smartir_cfg_to_dot(self, filename: str):
        """
        Export the CFG to a DOT file. The nodes includes the Solidity expressions and the IRs
        :param filename:
        :return:
        """
        content = self.smartir_cfg_to_dot_str()
        with open(filename, "w", encoding="utf8") as f:
            f.write(content)

    def smartir_cfg_to_dot_str(self, skip_expressions=False) -> str:
        """
    ...
    def _analyze_read_write(self):
        ...
        smartir_variables = [x.smartir_variables for x in self.nodes]
        smartir_variables = [x for x in smartir_variables if x]
        self._smartir_variables = [item for sublist in smartir_variables for item in sublist]
    ...
    def new_node(self, node_type: "NodeType", src: Union[str, Dict]) -> "Node":
        ...
        node.set_offset(src, self.smartfast)
    ...
    ###################################################################################
    ###################################################################################
    # region SmartIr and SSA
    ###################################################################################
    ###################################################################################

    def _get_last_ssa_variable_instances(
        self, target_state: bool, target_local: bool
    ) -> Dict[str, Set["SmartIRVariable"]]:
        # pylint: disable=too-many-locals,too-many-branches
        from smartfast.smartir.variables import ReferenceVariable
        from smartfast.smartir.operations import OperationWithLValue
        from smartfast.core.cfg.node import NodeType
    ...
    def get_last_ssa_state_variables_instances(self,) -> Dict[str, Set["SmartIRVariable"]]:
        return self._get_last_ssa_variable_instances(target_state=True, target_local=False)

    def get_last_ssa_local_variables_instances(self,) -> Dict[str, Set["SmartIRVariable"]]:
        return self._get_last_ssa_variable_instances(target_state=False, target_local=True)

    @staticmethod
    def _unchange_phi(ir: "Operation"):
        from smartfast.smartir.operations import Phi, PhiCallback
    ...
    def fix_phi(self, last_state_variables_instances, initial_state_variables_instances):
        from smartfast.smartir.operations import InternalCall, PhiCallback
        from smartfast.smartir.variables import Constant, StateIRVariable
    ...
    def generate_smartir_and_analyze(self):
        for node in self.nodes:
            node.smartir_generation()

        self._analyze_read_write()
        self._analyze_calls()

    @abstractmethod
    def generate_smartir_ssa(self, all_ssa_state_variables_instances):
        pass
