...
class SourceMapping(Context):
    ...
    @staticmethod
    def _compute_line(smartfast, filename, start: int, length: int) -> Tuple[List[int], int, int]:
        """
        ...
        """
        start_line, starting_column = smartfast.crytic_compile.get_line_from_offset(filename, start)
        end_line, ending_column = smartfast.crytic_compile.get_line_from_offset(
            filename, start + length
        )
        return list(range(start_line, end_line + 1)), starting_column, ending_column

    def _convert_source_mapping(self, offset: str, smartfast):  # pylint: disable=too-many-locals
        """
        ...
        """
        sourceUnits = smartfast.source_units
        ...
        if smartfast.crytic_compile:
            filenames = smartfast.crytic_compile.filename_lookup(filename_used)
            filename_absolute = filenames.absolute
            filename_relative = filenames.relative
            filename_short = filenames.short

            is_dependency = smartfast.crytic_compile.is_dependency(filename_absolute)

            if (
                filename_absolute in smartfast.source_code
                or filename_absolute in smartfast.crytic_compile.src_content
            ):
                filename = filename_absolute
            elif filename_relative in smartfast.source_code:
                filename = filename_relative
            elif filename_short in smartfast.source_code:
        ...
        if smartfast.crytic_compile:
            (lines, starting_column, ending_column) = self._compute_line(smartfast, filename, s, l)
        else:
    ...
    def set_offset(self, offset: Union[Dict, str], smartfast):
        if isinstance(offset, dict):
            self._source_mapping = offset
        else:
            self._source_mapping = self._convert_source_mapping(offset, smartfast)
