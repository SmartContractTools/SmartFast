...
from smartfast.smartir.operations import InternalDynamicCall, OperationWithLValue
...
def _is_vulnerable(node, variables_entrance):
    ...
    for ir_ssa in node.irs_ssa:
        if isinstance(ir_ssa, InternalDynamicCall):
            ...
            if isinstance(destination, SmartIRVariable) and ir_ssa.function.index == 0:
                return True
...
class UninitializedFunctionPtrsConstructor(AbstractDetector):
    ...
    WIKI = "https://github.com/SmartContractTools/SmartFast/wiki/Detector-Documentation#uninitialized-function-pointers-in-constructors"
...
The call to `a(10)` will lead to unexpected behavior because function pointer `a` is not initialized in the constructor."""
    ...
    def _detect(self):
        ...
        if self.smartfast.solc_version not in vulnerable_solc_versions:
            return results
