...
from smartfast.core.cfg.node import NodeType, Node
from smartfast.core.declarations import Function
from smartfast.core.expressions import UnaryOperation, UnaryOperationType
from smartfast.core.variables.variable import Variable
from smartfast.detectors.abstract_detector import AbstractDetector
from smartfast.smartir.operations import Call, EventCall,Assignment,TypeConversion,Unpack#,Send,Transfer
...
class AbstractState:
    ...
    def analyze_node(self, node, tainted_value_zero, detector):
        ...
        for internal_call in node.internal_calls:
            ...
            if isinstance(internal_call, Function):
                ...
                smartir_operations += internal_call.all_smartir_operations()

        contains_call = False

        self._written = state_vars_written
        for ir in node.irs + smartir_operations:
            ...
            # if detector.KEY == 'REENTRANCY_send_noeth':
            #     if detector.can_callback(ir) and not (isinstance(ir, (Send, Transfer)) and detector.can_send_eth(ir) and not ((isinstance(ir.call_value,Constant) and ir.call_value.value == 0) or ir.call_value.name in tainted_value_zero)):
            ...
            if detector.can_send_eth(ir) and not ((isinstance(ir.call_value,Constant) and ir.call_value.value == 0) or ir.call_value.name in tainted_value_zero):
...
class Reentrancy(AbstractDetector):
    ...
    def _explore(self, node, visited, tainted_value_zero, skip_father=None):
        """
        ...
        contains_call = fathers_context.analyze_node(node, tainted_value_zero, self)
        node.context[self.KEY] = fathers_context
        ...
        if contains_call and node.type in [NodeType.IF, NodeType.IFLOOP]:
            if self._filter_if(node):
                son = sons[0]
                self._explore(son, visited, tainted_value_zero, node)
                sons = sons[1:]
            else:
                son = sons[1]
                self._explore(son, visited, tainted_value_zero, node)
        ...
        for son in sons:
            self._explore(son, visited, tainted_value_zero)
        ...
        for function in contract.functions_and_modifiers_declared:
            ...
            if function.is_implemented:
                if self.KEY in function.context:
                    continue
                self._explore(function.entry_point, [],copy.deepcopy(tainted_value_zero))
