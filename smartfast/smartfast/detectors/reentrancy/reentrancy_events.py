...
class ReentrancyEvent(Reentrancy):
    ...
    WIKI = 'https://github.com/SmartContractTools/SmartFast/wiki/Detector-Documentation#reentrancy-vulnerabilities-events'
...
If `d.()` re-enters, the `Counter` events will be shown in an incorrect order, which might lead to issues for third parties.'''
    ...
    # @staticmethod
    # def can_callback(ir):
    #     return (isinstance(ir, Call) and ir.can_reenter()) or (isinstance(ir, (Send, Transfer)))
