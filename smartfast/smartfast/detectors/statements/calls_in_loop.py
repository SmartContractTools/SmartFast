...
If one of the destinations has a fallback function that reverts, `bad` will always revert.'''
    ...
    def detect_call_in_loop(contract):
        ...
        # print(contract.name)
        # print("**********")
        for f in contract.functions_and_modifiers_declared:#second modify function
            # print(f.name)
            # print(f.contract_declarer)
            if f.is_implemented:#third modify restriction
                # print(f.name)
                # print("---")
    ...
    def _detect(self):
        ...
        """
        results = []
        for c in self.contracts: #modify the contract
