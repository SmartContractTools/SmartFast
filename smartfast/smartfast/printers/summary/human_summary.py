...
from smartfast.core.declarations import SolidityFunction, Function
...
from smartfast.utils.colors import green, red, yellow
from smartfast.utils.myprettytable import MyPrettyTable
from smartfast.utils.standard_libraries import is_standard_library
from smartfast.core.cfg.node import NodeType
from smartfast.utils.tests_pattern import is_test_file
...
class PrinterHumanSummary(AbstractPrinter):
    ...
    WIKI = "https://github.com/trailofbits/smartfast/wiki/Printer-documentation#human-summary"
    ...
    def _get_detectors_result(self) -> Tuple[List[Dict], int, int, int, int, int]:
        ...
        checks_optimization = self.smartfast.detectors_optimization
        checks_informational = self.smartfast.detectors_informational
        checks_low = self.smartfast.detectors_low
        checks_medium = self.smartfast.detectors_medium
        checks_high = self.smartfast.detectors_high
    ...
    def _lines_number(self):
        if not self.smartfast.source_code:
            return None
        total_dep_lines = 0
        total_lines = 0
        total_tests_lines = 0

        for filename, source_code in self.smartfast.source_code.items():
            lines = len(source_code.splitlines())
            is_dep = False
            if self.smartfast.crytic_compile:
                is_dep = self.smartfast.crytic_compile.is_dependency(filename)
            if is_dep:
    ...
    def _compilation_type(self):
        ...
        return f"Compiled with {str(self.smartfast.crytic_compile.type)}\n"

    def _number_contracts(self):
        if self.smartfast.crytic_compile is None:
            return len(self.smartfast.contracts), 0, 0
        contracts = [c for c in self.smartfast.contracts if not c.is_top_level]
    ...
    def _get_features(self, contract):  # pylint: disable=too-many-branches
        ...
        for pragma in self.smartfast.pragma_directives:
            if (
        ...
        for function in contract.functions:
            ...
            for ir in function.smartir_operations:
