...
from smartfast.core.declarations import Modifier, Event, EnumContract, StructureContract, Function
...
from smartfast.solc_parsing.declarations.event import EventSolc
from smartfast.solc_parsing.declarations.function import FunctionSolc
from smartfast.solc_parsing.declarations.modifier import ModifierSolc
from smartfast.solc_parsing.declarations.structure_contract import StructureContractSolc
from smartfast.solc_parsing.exceptions import ParsingError, VariableNotFound
from smartfast.solc_parsing.solidity_types.type_parsing import parse_type
from smartfast.solc_parsing.variables.state_variable import StateVariableSolc
...
class ContractSolc:
    def __init__(self, smartfast_parser: "SmartfastSolc", contract: Contract, data):
        # assert smartfastSolc.solc_version.startswith('0.4')

        self._contract = contract
        self._contract.set_smartfast(smartfast_parser.core)
    ...
    def smartfast(self) -> "SmartfastCore":
        return self._contract.smartfast

    @property
    def smartfast_parser(self) -> "SmartfastSolc":
        return self._smartfast_parser
    ...
    def get_key(self) -> str:
        return self._smartfast_parser.get_key()
    ...
    def is_compact_ast(self) -> bool:
        return self._smartfast_parser.is_compact_ast
    ...
    def _parse_struct(self, struct: Dict):
        ...
        st.set_offset(struct["src"], self._contract.smartfast)
    ...
    def parse_state_variables(self):
        ...
        for varNotParsed in self._variablesNotParsed:
            var = StateVariable()
            var.set_offset(varNotParsed["src"], self._contract.smartfast)
    ...
    def _parse_modifier(self, modifier_data: Dict):
        modif = Modifier(self.smartfast)
        modif.set_offset(modifier_data["src"], self._contract.smartfast)
        modif.set_contract(self._contract)
        modif.set_contract_declarer(self._contract)

        modif_parser = ModifierSolc(modif, modifier_data, self, self.smartfast_parser)
        self._contract.smartfast.add_modifier(modif)
        self._modifiers_no_params.append(modif_parser)
        self._modifiers_parser.append(modif_parser)

        self._smartfast_parser.add_function_or_modifier_parser(modif_parser)
    ...
    def _parse_function(self, function_data: Dict):
        func = FunctionContract(self.smartfast)
        func.set_offset(function_data["src"], self._contract.smartfast)
        func.set_contract(self._contract)
        func.set_contract_declarer(self._contract)

        func_parser = FunctionSolc(func, function_data, self, self._smartfast_parser)
        self._contract.smartfast.add_function(func)
        self._functions_no_params.append(func_parser)
        self._functions_parser.append(func_parser)

        self._smartfast_parser.add_function_or_modifier_parser(func_parser)
    ...
    def log_incorrect_parsing(self, error):
        if self._contract.smartfast.disallow_partial:
    ...
    def _analyze_params_element(  # pylint: disable=too-many-arguments
        ...
        elem.set_offset(
            element_parser.function_not_parsed["src"], self._contract.smartfast,
        )

        elem_parser = Cls_parser(
            elem, element_parser.function_not_parsed, self, self.smartfast_parser
        )
        if (
        ...
        if isinstance(elem, Modifier):
            self._contract.smartfast.add_modifier(elem)
        else:
            self._contract.smartfast.add_function(elem)

        self._smartfast_parser.add_function_or_modifier_parser(elem_parser)
    ...
    def _analyze_params_elements(  # pylint: disable=too-many-arguments,too-many-locals
        ...
        try:
            for father in self._contract.inheritance:
                father_parser = self._smartfast_parser.underlying_contract_to_parser[father]
    ...
    def _analyze_enum(self, enum):
        ...
        new_enum.set_offset(enum["src"], self._contract.smartfast)
    ...
    def analyze_events(self):
        try:
            ...
            for event_to_parse in self._eventsNotParsed:
                ...
                event.set_offset(event_to_parse["src"], self._contract.smartfast)
