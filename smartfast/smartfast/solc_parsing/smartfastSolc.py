...
from smartfast.core.declarations.pragma_directive import Pragma
from smartfast.core.declarations.import_directive import Import
from smartfast.analyses.data_dependency.data_dependency import compute_dependency
from smartfast.solc_parsing.declarations.structure_top_level import StructureTopLevelSolc
from smartfast.solc_parsing.exceptions import VariableNotFound
from smartfast.solc_parsing.variables.top_level_variable import TopLevelVariableSolc

logging.basicConfig()
logger = logging.getLogger("SmartfastSolcParsing")
logger.setLevel(logging.INFO)


class SmartfastSolc:
    # pylint: disable=no-self-use,too-many-instance-attributes
    def __init__(self, filename: str, core: SmartfastCore):
        super().__init__()
    ...
    def parse_top_level_from_loaded_json(
        ...
        for top_level_data in data_loaded[self.get_children()]:
            if top_level_data[self.get_key()] == "ContractDefinition":
                ...
                raise SmartfastException(f"Top level {top_level_data[self.get_key()]} not supported")
    ...
    def parse_contracts(self):  # pylint: disable=too-many-statements,too-many-branches
        ...
        for contract in self._underlying_contract_to_parser:
            if (
                contract.name.startswith("SmartfastInternalTopLevelContract")
                and not contract.is_top_level
            ):
                raise SmartfastException(
                    """Your codebase has a contract named 'SmartfastInternalTopLevelContract'.
Please rename it, this name is reserved for Smartfast's internals"""
                )
    ...
    def analyze_contracts(self):  # pylint: disable=too-many-statements,too-many-branches
        if not self._parsed:
            raise SmartfastException("Parse the contract before running analyses")
        self._convert_to_smartir()
    ...
    def _analyze_top_level_structures(self):
        try:
            ...
            raise SmartfastException(f"Missing struct {e} during top level structure analyze") from e
    ...
    def _analyze_content_top_level_function(self):
        try:
            ...
            raise SmartfastException(f"Missing {e} during top level function analyze") from e
    ...
    def _convert_to_smartir(self):
        ...
        for contract in self._core.contracts:
            ...
            for func in contract.functions + contract.modifiers:
                try:
                    func.generate_smartir_and_analyze()
            ...
            contract.convert_expression_to_smartir_ssa()

        for func in self.core.functions_top_level:
            func.generate_smartir_and_analyze()
            func.generate_smartir_ssa(dict())
