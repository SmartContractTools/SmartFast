...
from smartfast.core.cfg.node import NodeType, Node, link_nodes
...
from smartfast.solc_parsing.yul.evm_functions import (
...
from smartfast.visitors.expression.find_calls import FindCalls
from smartfast.visitors.expression.read_var import ReadVar
from smartfast.visitors.expression.write_var import WriteVar
    ...
    def analyze_expressions(self):
        ...
        if self._node.expression:
            if self._node.type == NodeType.VARIABLE:
                ...
                _expression.set_offset(self._node.expression.source_mapping, self._node.smartfast)
...
class YulLocalVariable:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, var: LocalVariable, root: YulScope, ast: Dict):
        ...
        var.set_offset(ast["src"], root.smartfast)
...
class YulFunction(YulScope):
    ...
    def __init__(self, func: Function, root: YulScope, ast: Dict):
        ...
        func.set_offset(ast["src"], root.smartfast)
        func.set_contract(root.contract)
        func.smartfast = root.smartfast
    ...
    @property
    def smartfast(self) -> SmartfastCore:
        return self._entrypoint.underlying_node.smartfast
    ...
    def new_node(self, node_type, src) -> YulNode:
        if self._function:
            ...
            raise SmartfastException("standalone yul objects are not supported yet")
...
def parse_yul_function_call(root: YulScope, node: YulNode, ast: Dict) -> Optional[Expression]:
    ...
    if not isinstance(ident, Identifier):
        raise SmartfastException("expected identifier from parsing function name")
    ...
    raise SmartfastException(f"unexpected function call target type {str(type(ident.value))}")
...
def parse_yul_identifier(root: YulScope, _node: YulNode, ast: Dict) -> Optional[Expression]:
    ...
    raise SmartfastException(f"unresolved reference to identifier {name}")
...
def parse_yul_unsupported(_root: YulScope, _node: YulNode, ast: Dict) -> Optional[Expression]:
    raise SmartfastException(f"no parser available for {ast['nodeType']} {json.dumps(ast, indent=2)}")
...
def parse_yul(root: YulScope, node: YulNode, ast: Dict) -> Optional[Expression]:
    ...
    if op:
        op.set_offset(ast["src"], root.smartfast)
    return op
