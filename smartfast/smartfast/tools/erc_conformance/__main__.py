...
from smartfast.utils.erc import ERCS
...
logger = logging.getLogger("Smartfast-conformance")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Check the ERC 20 conformance", usage="smartfast-check-erc project contractName",
    )
...
def main():
    ...
    smartfast = Smartfast(args.project, **vars(args))

    ret = defaultdict(list)

    if args.erc.upper() in ERCS:

        contract = smartfast.get_contract_from_name(args.contract_name)
