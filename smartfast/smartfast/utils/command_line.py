...
from smartfast.detectors.abstract_detector import classification_txt
from smartfast.utils.colors import yellow, red
...
def output_detectors(detector_classes, detector_xml):
    detectors_list = []
    for detector in detector_classes:
        argument = detector.ARGUMENT
        # dont show the backdoor example
    ...
    # Sort by impact, confidence, and name
    print(detectors_list[0][2])
    detectors_list = sorted(
        detectors_list, key=lambda element: (element[2], element[3], element[0])
    )
    idx = 1
    for (argument, help_info, impact, confidence) in detectors_list:
        if argument in detector_xml:
            table.add_row([idx, argument, help_info, detector_xml[argument][0], detector_xml[argument][1]])
        else:
            table.add_row([idx, argument, help_info, classification_txt[impact], classification_txt[confidence]])
        idx = idx + 1
    print(table)
