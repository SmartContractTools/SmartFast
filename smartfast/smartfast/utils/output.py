...
from smartfast.core.declarations import Contract, Function, Enum, Event, Structure, Pragma
...
def output_to_zip(filename: str, error: Optional[str], results: Dict, zip_type: str = "lzma"):
    ...
    The file in the zip is named smartfast_results.json
    ...
    if os.path.isfile(filename):
        ...
        with ZipFile(
            ...
            file_desc.writestr("smartfast_results.json", json.dumps(json_result).encode("utf8"))
...
def _convert_to_description(d):
    ...
    if not isinstance(d, SourceMapping):
        raise SmartfastError(f"{d} does not inherit from SourceMapping, conversion impossible")
    ...
    raise SmartfastError(f"{type(d)} cannot be converted (no name, or canonical_name")
...
def _create_parent_element(element):
    ...
    from smartfast.core.children.child_inheritance import ChildInheritance
...
class Output:
    ...
    def add(self, add: SupportedOutput, additional_fields: Optional[Dict] = None):
        if isinstance(add, Variable):
            ...
            raise SmartfastError(f"Impossible to add {type(add)} to the json")
    ...
    def add_other(
        self, name: str, source_mapping, smartfast, additional_fields: Optional[Dict] = None,
    ):
        ...
        if isinstance(source_mapping, tuple):
            ...
            source_id = next(
                (
                    source_unit_id
                    for (source_unit_id, source_unit_filename,) in smartfast.source_units.items()
        ...
        if isinstance(source_mapping, str):
            ...
            source_mapping.set_offset(source_mapping_str, smartfast)
