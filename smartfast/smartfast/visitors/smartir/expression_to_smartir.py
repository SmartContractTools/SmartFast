...
from smartfast.core.solidity_types import ArrayType, ElementaryType
...
from smartfast.smartir.tmp_operations.argument import Argument
...
from smartfast.visitors.expression.expression import ExpressionVisitor

logger = logging.getLogger("VISTIOR:ExpressionToSmartIR")

key = "expressionToSmartIR"
...
def convert_assignment(left, right, t, return_type):
    ...
    raise SmartIRError("Missing type during assignment conversion")


class ExpressionToSmartIR(ExpressionVisitor):
    def __init__(self, expression, node):  # pylint: disable=super-init-not-called
        from smartfast.core.cfg.node import NodeType  # pylint: disable=import-outside-toplevel
    ...
    def _post_conditional_expression(self, expression):
        raise Exception("Ternary operator are not convertible to SmartIR {}".format(expression))
    ...
    def _post_unary_operation(
        ...
        if expression.type in [UnaryOperationType.BANG, UnaryOperationType.TILD]:
            ...
            raise SmartIRError("Unary operation to IR not supported {}".format(expression))
